...
public class NetworkHost extends Host {
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			hs.pkt.recievetime = MainEventManager.clock();
	...
	private void sendpackets() {
		...
		if (flag) {
			for (Vm vm : super.getVmList()) {
				vm.updateVmProcessing(MainEventManager.clock(), getVmScheduler().getAllocatedMipsForVm(vm));
			}
		}
		...
		for (NetworkPacket hs : packetTosendGlobal) {
			...
			MainEventManager.send(getDatacenter().getId(), sw.getId(), delay, CloudSimTags.Network_Event_UP, hs);
