...
public class NetworkCloudletSpaceSharedScheduler extends CloudletScheduler {
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) { // each machine in the
			...
			if ((cl.currStagenum != -1)) {
				...
				if (st.type == NetworkConstants.EXECUTION) {
					...
					// update the time
					cl.timespentInStage = Math.round(MainEventManager.clock() - cl.timetostartStage);
					...
					if (pktlist != null) {
						...
						if (it.hasNext()) {
							...
							if (pkt.reciever == cl.getVmId()) {
								pkt.recievetime = MainEventManager.clock();
								st.time = MainEventManager.clock() - pkt.sendtime;
								changetonextstage(cl, st);
								pkttoremove.add(pkt);
							}
						}
				...
				cl.currStagenum = 0;
				cl.timetostartStage = MainEventManager.clock();
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			...
			if (((NetworkCloudlet) (rcl.getCloudlet())).currStagenum == NetworkConstants.FINISH) {
				...
				((NetworkCloudlet) (rcl.getCloudlet())).finishtime = MainEventManager.clock();
				toRemove.add(rcl);
				cloudletFinish(rcl);
				finished++;
			}
	...
	public double cloudletResume(int cloudletId) {
		...
		if (found) {
			...
			if ((currentCpus - usedPes) >= rcl.getNumberOfPes()) {
				...
				double size = rcl.getRemainingCloudletLength();
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ncl.giacomobergami.components.cloudlet_scheduler.CloudletScheduler# getTotalCurrentAvailableMipsForCloudlet
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ncl.giacomobergami.components.cloudlet_scheduler.CloudletScheduler# getTotalCurrentAllocatedMipsForCloudlet
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ncl.giacomobergami.components.cloudlet_scheduler.CloudletScheduler# getTotalCurrentRequestedMipsForCloudlet
	 * (org.cloudbus.cloudsim.ResCloudlet, double)
	 */
	@Override
