...
public class HarddriveStorage implements Storage {
	 ...
	 */
	private void init() {
		fileList = new ArrayList<FileSIM>();
	 ...
	 * adding the file can also be found using {@link gridsim.datagrid.FileSIM#getTransactionTime()}.
	 * 
	 * @param file the file to be added
	 * @return the time (in seconds) required to add the file
	 */
	@Override
	public double addReservedFile(FileSIM file) {
		if (file == null) {
			return 0;
		}
	...
	public boolean hasPotentialAvailableSpace(int fileSize) {
		...
		Iterator<FileSIM> it = fileList.iterator();
		FileSIM file = null;
	 ...
	 * also be found using {@link gridsim.datagrid.FileSIM#getTransactionTime()}.
	 * 
	 * @param fileName the name of the needed file
	 * @return the file with the specified filename
	 */
	@Override
	public FileSIM getFile(String fileName) {
	 ...
	 * {@link gridsim.datagrid.FileSIM#getTransactionTime()}.
	 * 
	 * @param file the file to be added
	 * @return the time taken (in seconds) for adding the specified file
	 */
	@Override
	public double addFile(FileSIM file) {
		double result = 0.0;
	...
	public double addFile(List<FileSIM> list) {
		...
		Iterator<FileSIM> it = list.iterator();
		FileSIM file = null;
		while (it.hasNext()) {
			file = it.next();
	 ...
	 * be found using {@link gridsim.datagrid.FileSIM#getTransactionTime()}.
	 * 
	 * @param fileName the name of the file to be removed
	 * @return the deleted file
	 */
	@Override
	public FileSIM deleteFile(String fileName) {
		if (fileName == null || fileName.length() == 0) {
			return null;
		}
	 ...
	 */
	@Override
	public double deleteFile(String fileName, FileSIM file) {
	 ...
	 */
	@Override
	public double deleteFile(FileSIM file) {
		double result = 0.0;
		// check if the file is valid or not
	 ...
	 */
	@Override
	public boolean contains(FileSIM file) {
		boolean result = false;
	 ...
	 * found using {@link gridsim.datagrid.FileSIM#getTransactionTime()}.
	 * 
	 ...
	 */
	@Override
	public boolean renameFile(FileSIM file, String newName) {
		...
		FileSIM obj = getFile(file.getName());
		if (obj == null) {
			return result;
		} else {
			obj.setName(newName);
