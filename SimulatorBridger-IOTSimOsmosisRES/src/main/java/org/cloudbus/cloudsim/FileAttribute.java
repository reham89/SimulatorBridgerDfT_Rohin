...
/**
 * A class for storing related information regarding to a {@link gridsim.datagrid.FileSIM} entity.
 * 
 * @author Uros Cibej
...
public class FileAttribute {
	...
	public FileAttribute(String fileName, int fileSize) throws ParameterException {
		...
		Date date = MainEventManager.getSimulationCalendar().getTime();
		if (date == null) {
			creationTime = 0;
		} else {
		...
		ownerName = null;
		id = FileSIM.NOT_REGISTERED;
		checksum = 0;
		type = FileSIM.TYPE_UNKOWN;
		lastUpdateTime = 0;
		cost = 0;
		resourceId = -1;
		masterCopy = true;
